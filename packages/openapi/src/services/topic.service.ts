  ...
  async getTopics(request: TopicListQuery): Promise<TopicListResponse> {
    try {
      ...
      if (request.groupId) {
        ...
        // Look up sessionId via agentId, then filter by sessionId
        ...
        if (relation) {
          ...
          // No corresponding session for agentId, return empty directly
      ...
      // Exclude topics from specified trigger sources
      ...
      // If there is a keyword, add fuzzy search condition for title
      if (request.keyword) {
