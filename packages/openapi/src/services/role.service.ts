...
export class RoleService extends BaseService {
  ...
  async updateRolePermissions(
    ...
    // Resolve the intersection of grant and revoke; intersecting permissions cancel each other out, effectively a no-op
  ...
  async updateRole(id: string, updateData: UpdateRoleRequest): ServiceResult<RoleItem> {
    ...
    try {
      return await this.db.transaction(async (tx) => {
        ...
        // Check if it is a system role; system roles are not allowed to modify certain fields
        if (existingRole.isSystem && (updateData.name || updateData.isSystem === false)) {
          throw this.createBusinessError('系统角色不允许修改名称或系统属性');
        }

        // If updating the role name, check if the new name already exists
