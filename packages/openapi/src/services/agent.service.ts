...
export class AgentService extends BaseService {
  ...
  async queryAgents(request: GetAgentsRequest): ServiceResult<AgentListResponse> {
    ...
    try {
      // Base filter conditions: current user + exclude virtual agents (inbox, supervisor, etc.)
    ...
    try {
      return await this.db.transaction(async (tx) => {
        ...
        const newAgentData: NewAgent = {
          ...
          slug: randomSlug(4), // Automatically generated slug by the system
  ...
  async updateAgent(request: UpdateAgentRequest): ServiceResult<AgentDetailResponse> {
    ...
    try {
      ...
      return await this.db.transaction(async (tx) => {
        ...
        // Only update fields actually passed in the request, to avoid overwriting existing values with undefined
        ...
        // params uses merge update rather than full override
  ...
  async deleteAgent(request: AgentDeleteRequest): ServiceResult<void> {
    ...
    try {
      ...
      if (request.migrateSessionTo) {
        // Verify that the migration target Agent exists and belongs to the current user
        ...
        // After migration, delete the agent itself directly; sessions have been transferred, no cascade delete needed
        await this.db
          .delete(agents)
          .where(and(eq(agents.id, request.agentId), eq(agents.userId, this.userId)));
      } else {
        // No migration: reuse AgentModel.delete, which will cascade delete associated sessions, messages, topics, etc.
  ...
  async getAgentById(agentId: string): ServiceResult<AgentDetailResponse | null> {
    ...
    try {
      ...
      // Reuse AgentModel's method to get the complete Agent configuration
      const agentModel = new AgentModel(this.db, this.userId);
  ...
  /**
   * Migrate Agent sessions to another Agent
  ...
  private async migrateAgentSessions(fromAgentId: string, toAgentId: string): Promise<void> {
    ...
    try {
      await this.db.transaction(async (tx) => {
        ...
        // Delete source agent's association records, then insert association records pointing to the target agent
        // Directly updating agentId may violate unique constraints, so use delete + insert
        await tx
